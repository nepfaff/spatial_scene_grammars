"""
Expects the `greg_table` package to be present in the parent directory. This is stored
on TRI efs.
"""
...
from spatial_scene_grammars_examples.table.grammar import (
    ...
    ObjectSpacingConstraintStructure,
    Table,
    TallStackConstraint,
)
...
def extract_tree(tree: SceneTree, filter: bool) -> List[dict] | None:
    ...
    if filter:
        ...
        for node in observed_nodes:
            ...
            # objects = (
            #     spatial_scene_grammars_examples.table.grammar.Lamp,
            # )
...
def sample_realistic_scene(
    ...
    feasible_tree = project_tree_to_feasibility(
        deepcopy(good_tree),
        do_forward_sim=True,
        timestep=0.01,  # Most models use Hydro with primitves, allowing for the bigger timestep
        T=2.5,
        static_models="package://greg_table/models/misc/cafe_table/model.sdf",
    )
...
def main():
    ...
    constraint_list = [
        ObjectsOnTableConstraint(),
        ObjectSpacingConstraint(),
        # ObjectSpacingConstraintStructure(), # Rejection sampling will never succeed...
        TallStackConstraint(),
        NumStacksConstraint(),
    ]
